...
const Header = ({ onMenuClick, currentRoute }: HeaderProps) => {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'page': return 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400';
      case 'report': return 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400';
      case 'action': return 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-400';
      case 'setting': return 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-400';
  ...
  const handleSettingsNavigation = (tabId: string) => {
    console.log('Navigating to settings tab:', tabId);
    try {
  ...
  return (
    <header className="sticky top-0 z-30 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-6 lg:px-8 shadow-sm">
        ...
        <div className="flex-1 max-w-lg mx-8 hidden sm:block" ref={searchRef}>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-12 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all duration-200 bg-gray-50 dark:bg-gray-700 dark:text-gray-100 focus:bg-white dark:focus:bg-gray-600"
            />
            {searchQuery && (
              <button
                onClick={() => {
                  ...
                  <>
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {filteredResults.map((result, index) => (
                        <button
                          ...
                          className={`w-full px-4 py-3 flex items-start space-x-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-left ${
                            index === selectedResultIndex ? 'bg-indigo-50 dark:bg-indigo-900/30 border-l-4 border-l-indigo-500 dark:border-l-indigo-400' : 'border-l-4 border-l-transparent'
                          }`}
                        >
                    ...
                    <div className="px-4 py-2 bg-gray-50 dark:bg-gray-700 border-t border-gray-100 dark:border-gray-600">
                      <p className="text-xs text-gray-400 dark:text-gray-500">
                        Press <kbd className="px-1.5 py-0.5 bg-gray-200 dark:bg-gray-600 rounded text-gray-600 dark:text-gray-300 font-mono">↵</kbd> to select, <kbd className="px-1.5 py-0.5 bg-gray-200 dark:bg-gray-600 rounded text-gray-600 dark:text-gray-300 font-mono">Esc</kbd> to close
                      </p>
                    </div>
                  </>
                ) : (
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          ...
          <div className="relative hidden md:block">
            <select
              ...
              className="appearance-none bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 pr-8 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none cursor-pointer"
            >
                        ...
                        <div className="flex items-start space-x-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1">
                              ...
                              {!notification.isRead && (
                                <span className="flex-shrink-0 w-2 h-2 bg-indigo-500 dark:bg-indigo-400 rounded-full animate-pulse"></span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center justify-between mt-2">
                              ...
                              <span className={`text-xs px-2 py-0.5 rounded-full ${
                                notification.severity === 'high' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' :
                                notification.severity === 'medium' ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400' :
              ...
              <div className="absolute right-0 mt-2 w-56 sm:w-64 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
                  ...
                  {settingsMenuItems.map((item) => (
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log('Button clicked:', item.id, item.name);
                        handleSettingsNavigation(item.id);
                      }}
                      className="w-full flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 cursor-pointer text-left"
                    >
                ...
                <div className="border-t border-gray-100 dark:border-gray-700 pt-2">
                  <button
                    ...
                    className="w-full flex items-center px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors duration-200 cursor-pointer text-left"
                  >
