...
const LineChart: React.FC<LineChartProps> = ({
    ...
    const updateDimensions = () => {
      if (containerRef.current && responsive) {
        ...
        const calculatedWidth =
          typeof width === 'string' ? Math.max(containerWidth - 32, 320) : (width as number)

        const calculatedHeight =
          containerHeight > 0 && containerHeight !== height ? containerHeight : height
  ...
  useEffect(() => {
    ...
    const processedData = data.map((d) => ({
      ...d,
      [xKey]:
        typeof d[xKey] === 'string' && String(d[xKey]).match(/^\d{4}-\d{2}-\d{2}$/)
    ...
    const allYValues =
      series.length > 0
        ? processedData.flatMap((d) =>
            series.map((s) => d[s.key] as number).filter((v) => typeof v === 'number')
          )
    ...
    // Create scales
    const xScale =
      processedData[0][xKey] instanceof Date
        ? d3
            .scaleTime()
        ...
        if (showDots) {
          g.selectAll(`.dot-${index}`)
            ...
            .on('mouseover', function (event, d) {
              ...
              tooltip
                .html(
                  `${s.name}: ${d3.format(',')(d[s.key] as number)}<br/>${xAxisLabel}: ${d[xKey]}`
                )
    ...
    if (showLegend && series.length > 0) {
      ...
      const legendItemWidth = Math.min(
        140,
        (actualWidth - responsiveMargin.left - responsiveMargin.right) / series.length
      )
