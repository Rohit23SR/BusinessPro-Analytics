...
const AreaChart = ({
  ...
  useEffect(() => {
    ...
    if (animate) {
      // Animate area
      areaPath.attr('opacity', 0).transition().duration(1000).attr('opacity', 1)
    ...
    if (showDots) {
      ...
      if (animate) {
        dots.transition().delay(1500).duration(500).attr('r', 4)
      } else {
        dots.attr('r', 4)
      }
    }
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      >
        {hoveredData && (
          <div>
            <div className="mb-1 font-medium text-gray-900">{String(hoveredData[xKey])}</div>
            <div className="flex items-center space-x-2">
              <div className="h-3 w-3 rounded-full" style={{ backgroundColor: strokeColor }} />
              <span className="text-gray-600">Value: {d3.format(',')(hoveredData[yKey])}</span>
            </div>
          </div>
        )}
      </div>
