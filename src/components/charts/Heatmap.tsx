...
const HeatMap: React.FC<HeatMapProps> = ({
  ...
  useEffect(() => {
    ...
    const xScale = d3.scaleBand().domain(xValues.map(String)).range([0, innerWidth]).padding(0.02)

    const yScale = d3.scaleBand().domain(yValues).range([0, innerHeight]).padding(0.02)

    // Better color scale with more contrast
    const colorScale = d3.scaleSequential().interpolator(d3.interpolateBlues).domain([0, 100]) // Fixed domain for consistency
    ...
    g.selectAll('.cell')
      ...
      .on('mouseout', function () {
        ...
        d3.select(this).style('opacity', 1).style('stroke-width', '1px').style('stroke', '#ffffff')
      })
    ...
    if (typeof xValues[0] === 'number') {
      ...
      xAxis = d3.axisBottom(xScale).tickFormat((d) => {
        const hour = Number(d)
        if (hour % 3 === 0) {
          // Show every 3rd hour
          return hour === 0
