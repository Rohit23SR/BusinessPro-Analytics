...
const BarChart = ({
  ...
  useEffect(() => {
    ...
    // Color scale
    const colorScale = d3.scaleOrdinal<string>().domain(valueKeys).range(colors)
    ...
    if (orientation === 'vertical') {
      ...
      if (stacked) {
        // Stacked data
        const stackedData = d3.stack<BarChartData>().keys(valueKeys)(data)

        const maxY = d3.max(stackedData, (series) => d3.max(series, (d) => d[1])) || 0

        yScale = d3.scaleLinear().domain([0, maxY]).nice().range([innerHeight, 0])
        ...
        stackedData.forEach((series, seriesIndex) => {
          ...
          if (animate) {
            ...
            bars.attr('y', (d) => yScale(d[1])).attr('height', (d) => yScale(d[0]) - yScale(d[1]))
          }
      ...
      valueKeys.forEach((valueKey, keyIndex) => {
        ...
        if (animate) {
          ...
          bars.attr('x', 0).attr('width', (d) => xScale(Number(d[valueKey]) || 0))
        }
      })
    }
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div
        ref={tooltipRef}
        className="pointer-events-none absolute z-10 rounded-lg border border-gray-200 bg-white p-3 opacity-0 shadow-lg transition-opacity"
        style={{ fontSize: '12px' }}
      >
        {hoveredData && (
          <div>
            <div className="mb-1 font-medium text-gray-900">{hoveredData[categoryKey]}</div>
