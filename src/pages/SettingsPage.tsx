...
export default function SettingsPage() {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'profile':
        return <ProfileTab settings={settings} onSettingChange={handleSettingChange} />
      case 'dashboard':
        return <DashboardTab settings={settings} onSettingChange={handleSettingChange} />
      case 'notifications':
        return <NotificationsTab settings={settings} onSettingChange={handleSettingChange} />
      case 'integrations':
        return (
          <IntegrationsTab
  ...
  if (isError) {
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-center">
          <p className="mb-4 text-red-600">Failed to load settings from server</p>
          <p className="text-sm text-gray-500">Using local settings instead</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <NotificationPopup show={showPopup} message={popupMessage} onClose={hideNotification} />

      {isSaving && (
        <div className="pointer-events-none fixed right-6 top-20 z-[100] flex items-center space-x-2 rounded-lg bg-indigo-600 px-4 py-2 text-white shadow-lg">
      ...
      {saveError && !isSaving && (
        <div className="fixed right-6 top-20 z-[100] flex items-center space-x-2 rounded-lg bg-red-600 px-4 py-2 text-white shadow-lg">
      ...
      <div className="space-y-6">
        <SettingsHeader onExport={handleExportJSON} onReset={handleReset} />
