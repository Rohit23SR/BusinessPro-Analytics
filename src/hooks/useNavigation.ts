...
export const useNavigation = (): NavigationState => {
  ...
  const isActive = useCallback(
    (path: string) => navigation.isActiveRoute(location.pathname, path),
    [location.pathname]
  )
  ...
  const navigateToTimeframe = useCallback(
    (timeframe: TimeFrame) => {
      const newParams = new URLSearchParams(searchParams)
  ...
  const updateFilters = useCallback(
    (newFilters: Partial<typeof filters>) => {
      ...
      Object.entries(newFilters).forEach(([key, value]) => {
        if (value && value !== '') {
          newParams.set(
            key === 'trafficSource' ? 'traffic_source' : key === 'deviceType' ? 'device_type' : key,
            value
          )
        } else {
          newParams.delete(
...
export const useRouteData = () => {
  ...
  const shouldFetchDashboard = useMemo(
    () => location.pathname.startsWith('/dashboard') && !location.pathname.includes('/analytics'),
    [location.pathname]
  )
