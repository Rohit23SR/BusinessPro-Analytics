...
const mapBackendToFrontend = (
  ...
  return {
    ...
    revenueWidget:
      (dashboardConfig.widgets as Record<string, { enabled: boolean }>)?.revenueChart?.enabled ??
      true,
    trafficWidget:
      (dashboardConfig.widgets as Record<string, { enabled: boolean }>)?.trafficSources?.enabled ??
      true,
    conversionsWidget:
      (dashboardConfig.widgets as Record<string, { enabled: boolean }>)?.kpis?.enabled ?? true,

    // Notification settings
    emailAlerts: (notifications.emailNotifications as Record<string, boolean>)?.alerts ?? true,
    emailReports:
      (notifications.emailNotifications as Record<string, boolean>)?.weeklyReports ?? false,
    pushAlerts:
      (notifications.pushNotifications as Record<string, boolean>)?.realTimeAlerts ?? true,
    pushReports:
      (notifications.pushNotifications as Record<string, boolean>)?.systemUpdates ?? true,
    conversionThreshold:
      (notifications.alertThresholds as Record<string, number>)?.conversionRateDropPercentage ?? 15,
    trafficThreshold:
      (notifications.alertThresholds as Record<string, number>)?.trafficDropPercentage ?? 25,
...
const mapFrontendToBackendPreferences = (
  ...
  if (
    updatedSettings.analytics !== undefined ||
    updatedSettings.marketing !== undefined ||
    updatedSettings.sharing !== undefined
  ) {
    prefs.privacy = {
...
const mapFrontendToBackendDashboard = (
  ...
  if (
    updatedSettings.revenueWidget !== undefined ||
    updatedSettings.trafficWidget !== undefined ||
    updatedSettings.conversionsWidget !== undefined
  ) {
    config.widgets = {
      kpis: {
        enabled: updatedSettings.conversionsWidget ?? currentSettings.conversionsWidget,
        position: 1,
      },
      revenueChart: {
        enabled: updatedSettings.revenueWidget ?? currentSettings.revenueWidget,
        position: 2,
      },
      trafficSources: {
        enabled: updatedSettings.trafficWidget ?? currentSettings.trafficWidget,
        position: 3,
      },
...
const mapFrontendToBackendNotifications = (
  ...
  if (
    updatedSettings.conversionThreshold !== undefined ||
    updatedSettings.trafficThreshold !== undefined
  ) {
    notifs.alertThresholds = {
      revenueDropPercentage: 10,
      conversionRateDropPercentage:
        updatedSettings.conversionThreshold ?? currentSettings.conversionThreshold,
...
export const useSettings = () => {
  ...
  const updateDashboardMutation = useMutation({
    mutationFn: (config: Record<string, unknown>) =>
      settingsApiClient.updateDashboardConfig(config),
    onSuccess: () => {
  ...
  }, [useBackendApi, preferencesQuery.data, dashboardConfigQuery.data, notificationsQuery.data])
  ...
  const isLoading =
    useBackendApi &&
    (preferencesQuery.isLoading || dashboardConfigQuery.isLoading || notificationsQuery.isLoading)

  const isError =
    useBackendApi &&
    (preferencesQuery.isError || dashboardConfigQuery.isError || notificationsQuery.isError)
